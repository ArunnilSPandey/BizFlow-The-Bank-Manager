...
export const undoTransaction = async (
    ...
    try {
      ...
      await runTransaction(firestore, async (transaction) => {
        const { fromId, toId, amount, type, id: originalTxId } = txToUndo;
        ...
        const fromPlayer = fromPlayerDoc?.exists() ? { ...fromPlayerDoc.data() as Player, id: fromPlayerDoc.id } : null;
        const toPlayer = toPlayerDoc?.exists() ? { ...toPlayerDoc.data() as Player, id: toPlayerDoc.id } : null;

        const memo = `Undo: ${txToUndo.type} - ${txToUndo.memo}`;
        ...
        switch (type) {
          case 'player-to-player':
            if (fromPlayer && toPlayer && fromPlayerRef && toPlayerRef) {
              transaction.update(fromPlayerRef, { balance: fromPlayer.balance + amount });
              transaction.update(toPlayerRef, { balance: toPlayer.balance - amount });
              addTransactionToLog(batch, firestore, gameId, fromPlayer, { fromId: toId, toId: fromId, amount, type: 'undo', memo, originalTxId }, fromPlayer.balance + amount);
              addTransactionToLog(batch, firestore, gameId, toPlayer, { fromId: toId, toId: fromId, amount, type: 'undo', memo, originalTxId }, toPlayer.balance - amount);
            }
            break;
          ...
          case 'repay-loan':
            if (fromPlayer && fromPlayerRef) {
              transaction.update(fromPlayerRef, { balance: fromPlayer.balance + amount, loan: fromPlayer.loan + amount });
               addTransactionToLog(batch, firestore, gameId, fromPlayer, { fromId: toId, toId: fromId, amount, type: 'undo', memo, originalTxId }, fromPlayer.balance + amount);
            }
            break;
          ...
          case 'receive-from-bank':
            if (toPlayer && toPlayerRef) {
              transaction.update(toPlayerRef, { balance: toPlayer.balance - amount });
              addTransactionToLog(batch, firestore, gameId, toPlayer, { fromId: fromId, toId: toId, amount, type: 'undo', memo, originalTxId }, toPlayer.balance - amount);
            }
            break;
          ...
          case 'pass-start':
             if(toPlayer && toPlayerRef) {
                // This is a complex one. We revert the round and balance.
                // We don't revert the interest, as that's a separate transaction.
                transaction.update(toPlayerRef, { 
                    balance: toPlayer.balance - amount,
                    round: toPlayer.round - 1,
                });
                addTransactionToLog(batch, firestore, gameId, toPlayer, { fromId: fromId, toId: toId, amount, type: 'undo', memo, originalTxId }, toPlayer.balance - amount);
             }
             break;
          ...
          default:
            throw new Error(`Cannot undo transaction of type: ${type}`);
        }
      });
      ...
      const permError = new FirestorePermissionError({
        path: `games/${gameId}/players`,
        operation: 'write',
        requestResourceData: { transactionToUndo: txToUndo.id },
      });
      errorEmitter.emit('permission-error', permError);
      throw e;
    }
